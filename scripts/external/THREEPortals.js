/* license:MIT
 * https://github.com/zadvorsky/three.portals/tree/master/src
 */
class Portal extends THREE.Mesh {

  constructor(width, height) {

    const geometry = new THREE.PortalGeometry(width, height);
    const material = new THREE.PortalMaterial();

    super(geometry, material);

    this.type = "Portal";

    this.volumeFacesVisible = true;
    this.scene = null;
...
class PortalGeometry extends THREE.BufferGeometry {
  ...
  constructor(width, height) {
    ...
    // material groups
    this.addGroup(0, 6, 0); // front
    this.addGroup(6, indices.length - 6, 1); // volume
  }
  ...
  setVolume(fov, aspect, near) {
    ...
    this.applyMatrix4(this.sizeMatrix);
  }
}
...
THREE.Portal = Portal;
THREE.PortalGeometry = PortalGeometry;
